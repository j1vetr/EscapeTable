...
export default function Checkout() {
  ...
  const OrderSummaryCard = ({ isSticky = false }: { isSticky?: boolean }) => (
    <Card className={`p-6 border-2 ${isSticky ? "sticky top-4" : ""}`}>
          ...
          <div key={item.product.id} className="flex items-center gap-3 pb-3 border-b last:border-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground">
                {item.quantity} x {formatPrice(item.product.priceInCents)}
              </p>
            </div>
            <p className="text-sm font-semibold">{formatPrice(item.product.priceInCents * item.quantity)}</p>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-2 text-sm border-t pt-4">
        <div className="flex justify-between">
          <span className="text-muted-foreground">Ara Toplam ({items.length} ürün)</span>
        ...
        <div className="border-t pt-3 mt-3">
          <div className="flex justify-between items-center">
            <span className="font-semibold text-base">Toplam</span>
  ...
  return (
    <div className="pb-20">
      ...
      <div className="px-4 py-6 lg:grid lg:grid-cols-[minmax(0,1fr)_384px] gap-6 items-start">
        {/* Left: Checkout Steps */}
        ...
        {/* Right: Sticky Order Summary (hidden on mobile, sticky on desktop) */}
        <div className="hidden lg:block">
          <OrderSummaryCard isSticky={true} />
        </div>
      </div>

      {/* Mobile-only Order Summary (shown at bottom on mobile) */}
